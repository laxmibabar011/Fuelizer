...
const VehicleOnboardingStep: React.FC<VehicleOnboardingStepProps> = ({
  ...
  // Use prop partnerId if provided (embedded mode), otherwise use URL param (standalone mode)
  const partnerId = propPartnerId || urlPartnerId;
  ...
  useEffect(() => {
    ...
    const fetchPartner = async () => {
      try {
        const res = await creditService.getPartnerById(partnerId);
  ...
  const handleOnboardVehicles = async () => {
    ...
    try {
      ...
      if (isEmbedded && onComplete) {
        // Embedded mode - call onComplete callback
        setTimeout(() => {
          onComplete();
        }, 1500);
      } else {
        // Standalone mode - navigate to partner details
        setTimeout(() => {
              ...
              <TableBody>
                {vehicles.map((v, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${v.status === "Active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}
                      >
                        {v.status}
                      </span>
                    </TableCell>
