...
const ManagerManagement: React.FC = () => {
  ...
  const [_currentAssignments, setCurrentAssignments] = useState<
    Record<string, { id: string; shift_id: string }>
  ...
  const fetchManagers = async () => {
    try {
      ...
      setError(
        e?.response?.data?.error || e?.message || "Failed to load managers"
      );
    } finally {
      setLoading(false);
    }
  ...
  const loadTomorrowAssignments = async (
    _opts?: { id: string; label: string }[]
  ) => {
  ...
  const handleSaveAll = async () => {
    try {
      ...
      for (const m of managers) {
        ...
        if (selectedShiftId) {
          await staffshiftService.assignManagerShift({
            user_id: m.id,
            shift_id: selectedShiftId,
          });
        } else {
          // Unassign
          await staffshiftService.unassignManagerShift({
            user_id: m.id,
            shift_id: 0,
          });
        }
      ...
      setAssignError(
        e?.response?.data?.error || e?.message || "Failed to save assignments"
      );
    } finally {
      setAssigning(false);
    }
  ...
  if (loading)
    return <div className="p-4 text-sm text-gray-600">Loading managers...</div>;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {isEditing ? (
            <>
              {assignError && (
                <div className="text-sm text-red-600 mr-2">{assignError}</div>
              )}
                ...
                <div className="flex-1">
                  ...
                  {!isEditing ? (
                    <div className="mt-2 text-xs text-gray-500">
                      Assigned:{" "}
                      {m.DefaultManagerShift?.id
                        ? shiftOptions.find(
                            (o) => o.id === String(m.DefaultManagerShift?.id)
                          )?.label ||
